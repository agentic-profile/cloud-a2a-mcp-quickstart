...
const ArchiveVenture = () => {
    const { archive, archiveVenture, setVentureData, removeFromArchive, archiveName, setArchiveName } = useVentureStore();
    ...
    const saveLabel = archive.some(item => item.name === archiveName) ? 'Update' : 'Save';
                        ...
                        >
                            <div className="flex items-center gap-2">
                                <IconButton
                                    ...
                                    title="Open archive"
                                />
                                <span className={`text-sm text-gray-900 dark:text-gray-100 ${item.name === archiveName ? 'font-bold' : ''}`}>
                                    {item.name}
                                </span>
                            </div>
                            <IconButton
